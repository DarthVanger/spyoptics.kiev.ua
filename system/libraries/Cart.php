...
class CI_Cart {
	...
	public function __construct($params = array())
	{
		...
		{
			// No cart exists so we'll set some base values $this->_cart_contents['cart_total'] = 0;
	...
	function _insert($items = array())
	{
		...
		// Trying to fix the 'not adding the same product' bug. Author: DarthVanger@gmail.com
		// Now that we have our unique "row ID", we can check if we have the same item in the cart already. If so, just summ the quantities.
		if(array_key_exists($rowid, $this->_cart_contents)) {
			log_message('debug', 'product with rowid '.$rowid.' is already in the cart, so just summing up the quantities');
			$this->_cart_contents[$rowid]['qty'] += $items['qty'];
			return $rowid;
		}

		// Now add our cart items to the master array
	...
	/**
	 * Find item in cart by rowid
	 *
	 * Looks for the item by rowid and returns it as array. If nothing found, returns false.
	 *
	 * @access public
	 * @return array/false
	 */
